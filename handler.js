...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        if (global.db.data.settings[this.user.jid].gconly && !m.isGroup && !m.chat.endsWith('g.us') && !global.db.data.users[m.sender].premium) {
            if (!global.db.data.users[m.sender].lastReminderGconly || Date.now() - global.db.data.users[m.sender].lastReminderGconly >= 86400000) {
                global.db.data.users[m.sender].lastReminderGconly = Date.now();
        
            return conn.sendMessage(m.chat, {
                text: '`🚩 Akses Bot Ke Private Chat Ditolak!`\n\nAkses bot di private chat hanya tersedia untuk user *Premium!*\n\n📞 Hubungi Owner Untuk Membeli:\nketik *.owner*\n\n👥 Join Group Official: https://chat.whatsapp.com/HvJsTPuz1X02WygJ3l25Dc?mode=gi_t\n👥 Join Channel Official: https://whatsapp.com/channel/0029VbAzCrdCcW4i826fve3q',
                contextInfo: {
                      forwardingScore: 1,
                      isForwarded: true,
                      forwardedNewsletterMessageInfo: {
                        ...
                        serverMessageId: 103,
                        newsletterName: '[ + ] Follow Saluran Bot'
                      }
                    }
                  }, { quoted: m })
                  }
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `oh no limit mu habis, beli aja premium biar unlimited limit hehehe, ketik .buyprem kalo mau jadi user premium ._.`, m)
